...
namespace Vlc.DotNet.Wpf
{
    ...
    public sealed class VlcVideoProperties : IDisposable
    {
        ...
        public bool SetSubtitleFile(string subtitleFile)
        {
            if (!string.IsNullOrEmpty(subtitleFile) &&
                ...
                VlcContext.HandleManager.MediaPlayerHandles.ContainsKey(myHostVlcControl))
            {
               return VlcContext.InteropManager.MediaPlayerInterops.VideoInterops.SetSubtitleFile.Invoke(VlcContext.HandleManager.MediaPlayerHandles[myHostVlcControl],subtitleFile);
            }
            return false;
        }
